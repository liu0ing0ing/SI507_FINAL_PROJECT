        ...
        Tree = \
            ("Do you have specific restaurant?",
             ("Is it " + a_sort_dist[0]['name'] +"?",
              ('I got it', None, None),
              ('You want a closer restaurant?',
               ('Adjust distance for you', None, None),
               ('You want a restaurant with higher rating score?', ('Adjust rating for you', None, None),
                ('You want food delivery?', ('select delivery for you', None, None), ('select pickup for you', None, None))))),
             ('Do it again', None, None))

        # printTree(Tree)
        # write out tree in json
        tree_json = json.dumps(Tree)
        with open('tree_json.json', 'w') as outfile:
            outfile.write(tree_json)

        output = simplePlay(Tree)

        if output == 'Do it again':
